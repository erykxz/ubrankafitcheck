...
const POSE_INSTRUCTIONS = [
  "Widok od przodu, ręce na biodrach",
  "Lekki obrót, widok 3/4",
  "Widok z profilu",
  "Skok w powietrzu, ujęcie w ruchu",
  "Chód w kierunku aparatu",
  "Oparcie o ścianę",
];
...
const App: React.FC = () => {
  ...
  const handleGarmentSelect = useCallback(async (garmentFile: File, garmentInfo: WardrobeItem) => {
    ...
    setError(null);
    setIsLoading(true);
    setLoadingMessage(`Dodawanie ${garmentInfo.name}...`);
    ...
    try {
      ...
      setError(getFriendlyErrorMessage(err, 'Nie udało się nałożyć ubrania'));
    } finally {
      setIsLoading(false);
      setLoadingMessage('');
    }
  ...
  const handlePoseSelect = useCallback(async (newIndex: number) => {
    ...
    try {
      ...
      setError(getFriendlyErrorMessage(err, 'Nie udało się zmienić pozy'));
    ...
    <div className="font-sans">
      <AnimatePresence mode="wait">
        {!modelImageUrl ? (
          <motion.div
            key="start-screen"
            className="w-screen min-h-screen flex items-start sm:items-center justify-center bg-brand-bg p-4 pb-20"
          ...
          >
            <main className="flex-grow relative flex flex-col md:flex-row overflow-hidden">
              <div className="w-full h-full flex-grow flex items-center justify-center bg-brand-bg pb-16 relative">
                <Canvas 
              ...
              <aside 
                className={`absolute md:relative md:flex-shrink-0 bottom-0 right-0 h-auto md:h-full w-full md:w-1/3 md:max-w-sm bg-brand-light/80 backdrop-blur-md flex flex-col border-t md:border-t-0 md:border-l border-stone-300/60 transition-transform duration-500 ease-in-out ${isSheetCollapsed ? 'translate-y-[calc(100%-4.5rem)]' : 'translate-y-0'} md:translate-y-0`}
                style={{ transitionProperty: 'transform' }}
              >
                  <button 
                    onClick={() => setIsSheetCollapsed(!isSheetCollapsed)} 
                    className="md:hidden w-full h-8 flex items-center justify-center bg-stone-100/50"
                    aria-label={isSheetCollapsed ? 'Rozwiń panel' : 'Zwiń panel'}
                  >
                  ...
                  <div className="p-4 md:p-6 pb-20 overflow-y-auto flex-grow flex flex-col gap-8">
                    {error && (
                      <div className="bg-rose-100 border-l-4 border-rose-500 text-rose-800 p-4 mb-4 rounded-md" role="alert">
            ...
            <AnimatePresence>
              {isLoading && isMobile && (
                <motion.div
                  className="fixed inset-0 bg-brand-light/80 backdrop-blur-md flex flex-col items-center justify-center z-50"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                >
