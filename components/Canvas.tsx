...
const Canvas: React.FC<CanvasProps> = ({ displayImageUrl, onStartOver, isLoading, loadingMessage, onSelectPose, poseInstructions, currentPoseIndex, availablePoseKeys }) => {
  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-4 relative animate-zoom-in group">
      ...
      <button 
          onClick={onStartOver}
          className="absolute top-4 left-4 z-30 flex items-center justify-center text-center bg-brand-light/60 border border-stone-300/80 text-brand-dark font-semibold py-2 px-4 rounded-full transition-all duration-200 ease-in-out hover:bg-brand-light hover:border-stone-400 active:scale-95 text-sm backdrop-blur-sm"
      >
      ...
      <div className="relative w-full h-full flex items-center justify-center">
        {displayImageUrl ? (
          <img
            ...
            src={displayImageUrl}
            alt="Wirtualna modelka"
            className="max-w-full max-h-full object-contain transition-opacity duration-500 animate-fade-in rounded-lg"
          />
        ) : (
            <div className="w-[400px] h-[600px] bg-stone-100 border border-stone-200 rounded-lg flex flex-col items-center justify-center">
              <Spinner />
              <p className="text-md font-serif text-gray-600 mt-4">Ładowanie modelki...</p>
            </div>
        )}
        ...
        <AnimatePresence>
          {isLoading && (
              <motion.div
                  className="absolute inset-0 bg-brand-light/80 backdrop-blur-md flex flex-col items-center justify-center z-20 rounded-lg"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
              >
      ...
      {displayImageUrl && !isLoading && (
        ...
        >
          ...
          <AnimatePresence>
              {isPoseMenuOpen && (
                  <motion.div
                      ...
                      className="absolute bottom-full mb-3 w-64 bg-brand-light/90 backdrop-blur-lg rounded-xl p-2 border border-stone-300/80"
                  >
                      <div className="grid grid-cols-2 gap-2">
                          {poseInstructions.map((pose, index) => (
                              <button
                                  ...
                                  className="w-full text-left text-sm font-medium text-brand-dark p-2 rounded-md hover:bg-stone-200/80 disabled:opacity-50 disabled:bg-stone-200/80 disabled:font-bold disabled:cursor-not-allowed"
                              >
                                  {pose}
                              </button>
                          ))}
          ...
          <div className="flex items-center justify-center gap-2 bg-brand-light/60 backdrop-blur-md rounded-full p-2 border border-stone-300/50">
            <button 
              onClick={handlePreviousPose}
              aria-label="Poprzednia poza"
              className="p-2 rounded-full hover:bg-brand-light/90 active:scale-90 transition-all disabled:opacity-50"
              disabled={isLoading}
            >
              <ChevronLeftIcon className="w-5 h-5 text-brand-dark" />
            </button>
            <span className="text-sm font-semibold text-brand-dark w-48 text-center truncate" title={poseInstructions[currentPoseIndex]}>
              {poseInstructions[currentPoseIndex]}
            </span>
            <button 
              onClick={handleNextPose}
              aria-label="Następna poza"
              className="p-2 rounded-full hover:bg-brand-light/90 active:scale-90 transition-all disabled:opacity-50"
              disabled={isLoading}
            >
