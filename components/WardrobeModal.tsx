...
const WardrobePanel: React.FC<WardrobePanelProps> = ({ onGarmentSelect, activeGarmentIds, isLoading, wardrobe }) => {
    ...
    const handleGarmentClick = async (item: WardrobeItem) => {
        ...
        try {
            ...
            const detailedError = `Nie udało się załadować elementu szafy. Często jest to problem z CORS. Sprawdź konsolę deweloperską, aby uzyskać szczegółowe informacje.`;
  ...
  return (
    <div className="pt-6 border-t border-stone-400/50">
        <h2 className="text-xl font-serif tracking-wider text-brand-dark mb-3">Szafa</h2>
        <div className="grid grid-cols-3 gap-3">
            ...
            return (
                ...
                className="relative aspect-square border border-stone-300 rounded-lg overflow-hidden transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-dark group disabled:opacity-60 disabled:cursor-not-allowed"
                ...
                {isActive && (
                    <div className="absolute inset-0 bg-brand-dark/70 flex items-center justify-center">
                        <CheckCircleIcon className="w-8 h-8 text-brand-pink" />
                    </div>
                )}
                </button>
            );
            })}
            <label htmlFor="custom-garment-upload" className={`relative aspect-square border-2 border-stone-300 border-dashed rounded-lg flex flex-col items-center justify-center text-stone-500 transition-colors ${isLoading ? 'cursor-not-allowed bg-stone-100' : 'hover:border-brand-pink hover:text-brand-pink cursor-pointer'}`}>
                <UploadCloudIcon className="w-6 h-6 mb-1"/>
                <span className="text-xs text-center">Wgraj</span>
        ...
        {wardrobe.length === 0 && (
             <p className="text-center text-sm text-gray-500 mt-4">Wgrane przez Ciebie ubrania pojawią się tutaj.</p>
        )}
