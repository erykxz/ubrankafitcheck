...
export function getFriendlyErrorMessage(error: unknown, context: string): string {
    let rawMessage = 'Wystąpił nieznany błąd.';
    ...
    if (rawMessage.includes("Unsupported MIME type")) {
        try {
            ...
            if (nestedMessage && nestedMessage.includes("Unsupported MIME type")) {
                ...
                return `Typ pliku '${mimeType}' nie jest obsługiwany. Użyj formatu takiego jak PNG, JPEG lub WEBP.`;
            }
        } catch (e) {
        ...
        return `Nieobsługiwany format pliku. Wgraj plik w formacie PNG, JPEG lub WEBP.`;
    }
